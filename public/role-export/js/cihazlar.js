export const CIHAZ_PROFILLERI = {
  "quansheng-uv-k5-f4hwn": {
    ad: "Quansheng UV-K5 (F4HWN v4.3)",
    aciklama: "Custom firmware | Analog | FM/AM/USB | 18MHz-1.3GHz RX",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog"],
    maxKanal: 200,
    csvFormat: "chirp",
    csvSutunlari: [
      "Location", "Name", "Frequency", "Duplex", "Offset", "Tone",
      "rToneFreq", "cToneFreq", "DtcsCode", "DtcsPolarity", "RxDtcsCode",
      "CrossMode", "Mode", "TStep", "Skip", "Power", "Comment",
      "URCALL", "RPT1CALL", "RPT2CALL", "DVCODE",
    ],
    varsayilanDegerler: {
      tStep: "12.50",
      dtcsCode: "023",
      dtcsPolarity: "NN",
      rxDtcsCode: "023",
      crossMode: "Tone->Tone",
      mode: "FM",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      High: "5.0W",
      Mid: "2.0W",
      Low5: "1.0W",
      Low4: "0.5W",
      Low3: "0.3W",
      Low2: "0.2W",
      Low1: "0.1W",
    },
    maxKanalAdi: 10,
    ekOzellikler: {
      fmRadyo: true,
      airBand: true,
      bandscope: true,
      ssbDemod: true,
      marineBand: true,
      genisRX: "18MHz - 1.3GHz",
      txBant: "VHF (136-174) / UHF (400-470)",
    },
  },
  "quansheng-uv-k5v3-uv-k1-f4hwn": {
    ad: "Quansheng UV-K5 v3 / UV-K1 (F4HWN v5.3.1)",
    aciklama: "Custom firmware | Analog | FM/AM/USB | 18MHz-1.3GHz RX",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog"],
    maxKanal: 1000,
    csvFormat: "chirp",
    csvSutunlari: [
      "Location", "Name", "Frequency", "Duplex", "Offset", "Tone",
      "rToneFreq", "cToneFreq", "DtcsCode", "DtcsPolarity", "RxDtcsCode",
      "CrossMode", "Mode", "TStep", "Skip", "Power", "Comment",
      "URCALL", "RPT1CALL", "RPT2CALL", "DVCODE",
    ],
    varsayilanDegerler: {
      tStep: "12.50",
      dtcsCode: "023",
      dtcsPolarity: "NN",
      rxDtcsCode: "023",
      crossMode: "Tone->Tone",
      mode: "FM",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      High: "5.0W",
      Mid: "2.0W",
      Low5: "1.0W",
      Low4: "0.5W",
      Low3: "0.3W",
      Low2: "0.2W",
      Low1: "0.1W",
    },
    maxKanalAdi: 10,
    ekOzellikler: {
      fmRadyo: true,
      airBand: true,
      bandscope: true,
      ssbDemod: true,
      marineBand: true,
      genisRX: "18MHz - 1.3GHz",
      txBant: "VHF (136-174) / UHF (400-470)",
    },
  },
  "baofeng-k5-plus": {
    ad: "Baofeng K5 Plus",
    aciklama: "Tri-Band Analog | FM/AM | 65-520MHz RX | 10W",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog"],
    maxKanal: 999,
    csvFormat: "chirp",
    csvSutunlari: [
      "Location", "Name", "Frequency", "Duplex", "Offset", "Tone",
      "rToneFreq", "cToneFreq", "DtcsCode", "DtcsPolarity", "RxDtcsCode",
      "CrossMode", "Mode", "TStep", "Skip", "Power", "Comment",
      "URCALL", "RPT1CALL", "RPT2CALL", "DVCODE",
    ],
    varsayilanDegerler: {
      tStep: "12.50",
      dtcsCode: "023",
      dtcsPolarity: "NN",
      rxDtcsCode: "023",
      crossMode: "Tone->Tone",
      mode: "FM",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      High: "10.0W",
      Mid: "7.0W",
      Low: "4.0W",
    },
    maxKanalAdi: 8,
    ekOzellikler: {
      fmRadyo: true,
      airBand: true,
      bandscope: false,
      ssbDemod: false,
      marineBand: false,
      genisRX: "65MHz - 520MHz",
      txBant: "VHF (136-174) / 1.25m (220-260) / UHF (400-480)",
    },
  },
  "opengd77": {
    ad: "OpenGD77 (Radioddity GD-77 / DM-1801 / RD-5R)",
    aciklama: "OpenGD77 Custom Firmware | DMR + Analog | VHF/UHF",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog", "Dijital"],
    maxKanal: 1023,
    csvFormat: "opengd77",
    csvSutunlari: [
      "Channel Number", "Channel Name", "Channel Type", "Rx Frequency", "Tx Frequency",
      "Bandwidth (kHz)", "Colour Code", "Timeslot", "Contact", "TG List", "DMR ID",
      "TS1_TA_Tx", "TS2_TA_Tx ID", "RX Tone", "TX Tone", "Squelch", "Power",
      "Rx Only", "Zone Skip", "All Skip", "TOT", "VOX", "No Beep", "No Eco",
      "APRS", "Latitude", "Longitude", "Use Location",
    ],
    varsayilanDegerler: {
      bandwidth: "12.5",
      colourCode: "1",
      timeslot: "1",
      tgList: "None",
      dmrId: "None",
      squelch: "Disabled",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      Master: "Radio Master",
      P9: "P9 (max)",
      P8: "P8",
      P7: "P7",
      P6: "P6",
      P5: "P5",
      P4: "P4",
      P3: "P3",
      P2: "P2",
      P1: "P1 (min)",
      "-W+": "-W+ (user)",
    },
    maxKanalAdi: 16,
    ekOzellikler: {
      fmRadyo: false,
      airBand: false,
      bandscope: false,
      ssbDemod: false,
      marineBand: false,
      genisRX: "VHF/UHF",
      txBant: "VHF (136-174) / UHF (400-480)",
    },
  },
  "tyt-md-uv390-plus": {
    ad: "TYT MD-UV390 Plus",
    aciklama: "DMR Dijital + Analog | VHF/UHF",
    bantlar: ["VHF", "UHF", "APRS"],
    modlar: ["Analog", "Dijital"],
    maxKanal: 3000,
    csvFormat: "cps",
    csvSutunlari: [
      "Channel Mode", "Channel Name", "RX Frequency(MHz)", "TX Frequency(MHz)",
      "Band Width", "Scan List", "Squelch", "RX Ref Frequency", "TX Ref Frequency",
      "TOT[s]", "TOT Rekey Delay[s]", "Power", "Admit Criteria", "Auto Scan",
      "Rx Only", "Lone Worker", "VOX", "Allow Talkaround", "Send GPS Info",
      "Receive GPS Info", "Private Call Confirmed", "Emergency Alarm Ack",
      "Data Call Confirmed", "Allow Interrupt", "DCDM Switch", "Leader/MS",
      "Emergency System", "Contact Name", "Group List", "Color Code",
      "Repeater Slot", "In Call Criteria", "Privacy", "Privacy No.", "GPS System",
      "CTCSS/DCS Dec", "CTCSS/DCS Enc", "Rx Signaling System", "Tx Signaling System",
      "QT Reverse", "Non-QT/DQT Turn-off Freq", "Display PTT ID",
      "Reverse Burst/Turn-off Code", "Decode 1", "Decode 2", "Decode 3",
      "Decode 4", "Decode 5", "Decode 6", "Decode 7", "Decode 8",
    ],
    varsayilanDegerler: {
      bandWidth: "0", squelch: "3", tot: "4", power: "2",
      leaderMS: "1", contactName: "1", groupList: "1", colorCode: "1",
      nonQtDqt: "2", displayPtt: "1", reverseBurst: "1",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    gucSeviyeleri: { High: "2", Mid: "1", Low: "0" },
    maxKanalAdi: 16,
    varsayilanKanalFormati: "plaka-bant-konum",
  },
  "tyt-md-uv390": {
    ad: "TYT MD-UV390",
    aciklama: "DMR Dijital + Analog | VHF/UHF (GPS'siz)",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog", "Dijital"],
    maxKanal: 3000,
    csvFormat: "cps",
    csvSutunlari: [
      "Channel Mode", "Channel Name", "RX Frequency(MHz)", "TX Frequency(MHz)",
      "Band Width", "Scan List", "Squelch", "RX Ref Frequency", "TX Ref Frequency",
      "TOT[s]", "TOT Rekey Delay[s]", "Power", "Admit Criteria", "Auto Scan",
      "Rx Only", "Lone Worker", "VOX", "Allow Talkaround", "Send GPS Info",
      "Receive GPS Info", "Private Call Confirmed", "Emergency Alarm Ack",
      "Data Call Confirmed", "Allow Interrupt", "DCDM Switch", "Leader/MS",
      "Emergency System", "Contact Name", "Group List", "Color Code",
      "Repeater Slot", "In Call Criteria", "Privacy", "Privacy No.", "GPS System",
      "CTCSS/DCS Dec", "CTCSS/DCS Enc", "Rx Signaling System", "Tx Signaling System",
      "QT Reverse", "Non-QT/DQT Turn-off Freq", "Display PTT ID",
      "Reverse Burst/Turn-off Code", "Decode 1", "Decode 2", "Decode 3",
      "Decode 4", "Decode 5", "Decode 6", "Decode 7", "Decode 8",
    ],
    varsayilanDegerler: {
      bandWidth: "0", squelch: "3", tot: "4", power: "2",
      leaderMS: "1", contactName: "1", groupList: "1", colorCode: "1",
      nonQtDqt: "2", displayPtt: "1", reverseBurst: "1",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    gucSeviyeleri: { High: "2", Mid: "1", Low: "0" },
    maxKanalAdi: 16,
    varsayilanKanalFormati: "plaka-bant-konum",
  },
  "radtel-rt-910-bt": {
    ad: "Radtel RT-910 BT",
    aciklama: "Bluetooth | Analog | FM/AM | 45-520MHz RX | 960 Kanal",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog"],
    maxKanal: 960,
    csvFormat: "chirp",
    csvSutunlari: [
      "Location", "Name", "Frequency", "Duplex", "Offset", "Tone",
      "rToneFreq", "cToneFreq", "DtcsCode", "DtcsPolarity", "RxDtcsCode",
      "CrossMode", "Mode", "TStep", "Skip", "Power", "Comment",
      "URCALL", "RPT1CALL", "RPT2CALL", "DVCODE",
    ],
    varsayilanDegerler: {
      tStep: "12.50",
      dtcsCode: "023",
      dtcsPolarity: "NN",
      rxDtcsCode: "023",
      crossMode: "Tone->Tone",
      mode: "FM",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      High: "5.0W",
      Mid: "2.5W",
      Low: "1.0W",
    },
    maxKanalAdi: 12,
    ekOzellikler: {
      fmRadyo: true,
      airBand: true,
      bandscope: false,
      ssbDemod: false,
      marineBand: true,
      genisRX: "45MHz - 520MHz",
      txBant: "VHF (136-260) / UHF (300-520)",
    },
  },
  "baofeng-uv-21-pro-v2": {
    ad: "Baofeng UV-21 Pro V2",
    aciklama: "Tri-Band Analog | FM | 65-520MHz RX | 999 Kanal",
    bantlar: ["VHF", "UHF"],
    modlar: ["Analog"],
    maxKanal: 999,
    csvFormat: "chirp",
    csvSutunlari: [
      "Location", "Name", "Frequency", "Duplex", "Offset", "Tone",
      "rToneFreq", "cToneFreq", "DtcsCode", "DtcsPolarity", "RxDtcsCode",
      "CrossMode", "Mode", "TStep", "Skip", "Power", "Comment",
      "URCALL", "RPT1CALL", "RPT2CALL", "DVCODE",
    ],
    varsayilanDegerler: {
      tStep: "12.50",
      dtcsCode: "023",
      dtcsPolarity: "NN",
      rxDtcsCode: "023",
      crossMode: "Tone->Tone",
      mode: "FM",
    },
    shiftHesaplama: { VHF: -0.600, UHF: -7.600 },
    varsayilanKanalFormati: "plaka-bant-konum-kisa",
    gucSeviyeleri: {
      High: "5.0W",
      Low: "1.0W",
    },
    maxKanalAdi: 12,
    ekOzellikler: {
      fmRadyo: true,
      airBand: true,
      bandscope: false,
      ssbDemod: false,
      marineBand: true,
      genisRX: "65MHz - 520MHz",
      txBant: "VHF (136-174) / 1.25m (220-260) / UHF (400-480)",
    },
  },
};

export function cihazListesi() {
  return Object.entries(CIHAZ_PROFILLERI).map(([id, profil]) => ({ id, ad: profil.ad }));
}

export function cihazProfili(id) {
  return CIHAZ_PROFILLERI[id] ?? null;
}
