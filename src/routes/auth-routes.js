...
auth.post('/sifre-sifirla/:token', async (c) => {
  ...
  const salt = generateSalt();
  const hash = await hashPassword(password, salt);
  // token_version inkrement: eski cookie'ler invalid olsun.
  await db.prepare('UPDATE users SET password_hash = ?, password_salt = ?, password_iterations = ?, password_reset_token = NULL, password_reset_expires_at = NULL, token_version = token_version + 1 WHERE id = ?').bind(hash, salt, CURRENT_ITERATIONS, user.id).run();
...
auth.post('/profil/sifre', async (c) => {
  ...
  // token_version'i inkrement et: diger cihazlardaki eski cookie'ler invalid olsun.
  await db.prepare('UPDATE users SET password_hash = ?, password_salt = ?, password_iterations = ?, token_version = token_version + 1 WHERE id = ?').bind(hash, salt, CURRENT_ITERATIONS, userId).run();
