...
faq:
  - q: '`Failed to communicate with radio`'
    a: 'Sürücü, port, kablo'
  - q: Yarıda kesildi
    a: 'Pil zayıf, kablo gevşek'
  - q: Bazı kanallar boş
    a: CSV ayraç hatası
  - q: '`Module load failed` (CHIRP''te)'
    a: Yanlış driver dosyası
  - q: Telsiz F4HWN açıyor ama kanal yok
    a: İlk yüklemeden sonra fabrika reset gerekiyor
  - q: AM kanalı çatırdıyor
    a: Stock RX yetersiz
  - q: Spektrum donuk
    a: Spectrum mode buggy
extra_json_ld:
  - '@context': 'https://schema.org'
    '@type': HowTo
    name: Quansheng UV-K5 Programlama Rehberi (F4HWN Firmware)
    description: >-
      Quansheng UV-K5'i F4HWN custom firmware ile flashlemek, armel'in özel
      CHIRP driver'ı ile programlamak, röle CSV yüklemek ve EEPROM yedeği almak
      için tam rehber.
    inLanguage: tr-TR
    totalTime: PT40M
    step:
      - '@type': HowToStep
        position: 1
        name: EEPROM yedeği al (atlamayın)
        text: >-
          F4HWN'i flashlamadan **stock EEPROM'u yedekle** — geri dönmek
          istediğinizde gerekecek (kalibrasyon değerleri, fabrika frekans
          ofsetleri).
          ...
          (`-f` = full read, `-r` = read mode, `-B` = bin dosyası.) Yaklaşık 2-3
          dakika sürer, 64 KB dosya çıkar. Bu dosyayı **kaybetme** — bulutta
          yedekle.
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-1-eeprom-yedegi-al-atlamayin
      - '@type': HowToStep
        position: 2
        name: F4HWN firmware'i flashla
        text: >-
          ### Yöntem A: Web flasher (en kolay, Chrome/Edge)


          1. Telsizi **kapat → PTT basılı tutarak aç** → ekranda "Bootloader
          Mode" görünür.

          2. Programlama kablosunu USB'ye tak.

          3. [whosmatt.github.io/uvmod](https://whosmatt.github.io/uvmod/) →
          **"Connect"** → portu seç.

          4. **"Choose firmware"** → armel repo'sunun
          [Releases](https://github.com/armel/uv-k5-firmware-custom/releases)
          sayfasından en yeni `.bin` dosyasını seç (örn. `f4hwn_v5.2.0.bin`).

          5. **"Flash"** → ~30 sn → tamamlandığında telsiz kendili
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-2-f4hwn-firmware-i-flashla
      - '@type': HowToStep
        position: 3
        name: armel'in özel CHIRP driver'ını yükle
        text: >-
          ...
          → **Download ZIP** veya `git clone`.

          2. ZIP içindeki `uvk5_egzumer_f4hwn_v4_x.py` dosyasını CHIRP'in modül
          dizinine kopyala:
             ...
             - Veya CHIRP içinden: `Help` → `Load Module...` → `.py` d
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-3-armel-in-ozel-chirp-driver-ini-yukle
      - '@type': HowToStep
        position: 4
        name: Telsizden mevcut konfigürasyonu indir
        text: >-
          1. Programlama kablosunu bağla (telsiz açık, normal modda — bootloader
          değil).
          ...
          > **İndir başarısızsa:** Cihazda menüden `Settings → Reset` yapma — bu
          kalibrasyonu siler. Önce kabloyu çıkar/tak, sürücüyü kontrol et, USB
          hub yerine doğrudan bilgisayara bağla.
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-4-telsizden-mevcut-konfigurasyonu-indir
      - '@type': HowToStep
        position: 5
        ...
        text: >-
          ...
          1. **Cihaz**: "Quansheng UV-K5 (F4HWN v4.3+)" veya "UV-K1 / K5 v3
          (Fusion v5+, 1000 kanal)"

          2. **TA bölgesi**: ikamet ettiğiniz ile uygun bölge (TA1, TA2, ...)

          3. **Bant**: VHF + UHF

          4. **Mod**: Analog FM (DMR rölelerinizi ayrı tutun, kanal slotu
          kabarmaz)
        ...
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-5-role-csv-sini-hazirla
      - '@type': HowToStep
        position: 6
        name: CSV'yi CHIRP'e yükle
        text: >-
          1. CHIRP'te **File → Import** (yeni image gerekirse: New → Quansheng
          UV-K5 model'i).

          2. `roleler.csv` seç.

          3. CHIRP sütun eşleştirmesi sorabilir; röle export'umuz CHIRP standart
          sütunlarını kullandığı için otomatik tanır:
             - `Frequency` → RX
        ...
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-6-csv-yi-chirp-e-yukle
      - '@type': HowToStep
        position: 7
        ...
        text: >-
          ...
          | **Tone Squelch** (RX ton filtresi) | OFF — röleden gelen her şeyi
          duymak için |

          | **DTMF/PTT-ID** | OFF — yanlışlıkla DTMF kod göndermeyi engeller |

          | **TX Power** | 1W default; mesafe gerektikçe 5W'a çıkar (pil ömrü 2×
          fark eder) |

          | **Bandwidth** | NFM (12.5 kHz) amatör standardı; AM kanallarında AM
          |

          | **Skip** | Acil/zayıf röleleri `Skip = S` yaparak taramada
          hızlanırsınız |

          | **Squelch** | 3-5 (10 = en sıkı, 0 = süre
        url: 'https://amator.tr/tutorials/uv-k5-programlama#adim-7-manuel-ince-ayar'
      - '@type': HowToStep
        position: 8
        ...
        text: >-
          ...
          > **Yarıda kesilirse:** Pil seviyesi düşük olabilir; şarj edip tekrar
          deneyin. Yarım yüklemeden sonra cihaz garip davranabilir — yeniden tam
          upload yapın, kalıcı hasar olası değil.
        url: >-
          https://amator.tr/tutorials/uv-k5-programlama#adim-8-telsize-geri-yukle
---
